...
func _display_help(caller: CommandApplication, cmd: Command) -> void:
    ...
    for arg: CommandInputArgument in cmd.get_definition(caller)._arguments:
        var mode: String = TextEffectWrapper.standalone_tag(arg.get_mode_name())
        if arg.is_optional():
            mode = TextEffectWrapper.new(mode).color(Color.GRAY).get_value()

        text += mode + " " + arg.name + ": " + arg.description + "\n"
